# saga-http-client-injection Specification

## Purpose
TBD - created by archiving change saga-injectable-http-client. Update Purpose after archive.
## Requirements
...
注入的 `*http.Client` MUST NOT 影响 SAGA 现有的 trace 注入（`trace.Inject`）和 AK/SK 签名（`signRequestIfNeeded`）行为。
HTTPS 传输由底层 client/transport 承载，签名和 trace 在 `*http.Request` 请求头层面叠加，与 client 实例无关。
