# taskqueue-reply-example Specification

## Purpose
TBD - created by archiving change taskqueue-reply-example. Update Purpose after archive.
## Requirements
### Requirement: Producer 和 Consumer SHALL 分离部署
Producer 和 Consumer MUST 是独立的可执行程序，各自有自己的 main.go 入口。
...
### Requirement: 队列命名 SHALL 使用冒号分隔符
队列名称由多个语义部分组成时，MUST 使用冒号 `:` 作为分隔符。
...
### Requirement: Producer SHALL 指定回复队列
Producer 发送任务时，MUST 通过 ReplySpec 指定 Consumer 回复结果的队列名称。
...
### Requirement: Payload SHALL 包含 task_id 字段
任务的 Payload MUST 包含名为 `task_id` 的字符串字段，用于端到端任务匹配。
...
### Requirement: Example SHALL 演示 Inspector 功能
示例 MUST 展示 Inspector 接口的核心功能：查询队列列表、获取队列统计、查看任务详情。
...
### Requirement: Example SHALL 提供基于 Docker 的测试环境
示例 MUST 提供 Docker Compose 配置，用于启动测试所需的 Redis 服务。
