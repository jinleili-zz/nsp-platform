...
- [x] 1.2 Export and document `ErrTransactionFailed` and `ErrTransactionDisappeared`, and define return semantics for transaction failure, disappeared transactions, caller context cancellation, and infrastructure errors.
...
- [x] 1.4 Define wait-loop handling for transient `Query` errors, persistent `Query` errors, and disappeared transactions after successful submit, using package-private retry thresholds and backoff constants.
...
- [x] 3.1 Update `AGENTS.md` to document the new exported saga engine API, `ErrTransactionFailed`, `ErrTransactionDisappeared`, `SubmitAndWait` concurrent-safe usage, the `ctx` versus `WithTimeout` distinction, and that sentinel errors may be wrapped so callers SHOULD use `errors.Is`.
- [x] 3.2 Update `docs/saga.md`, `docs/modules/saga.md`, and `saga/README.md` with `SubmitAndWait` usage, return semantics, and when to choose it over `Submit`.
